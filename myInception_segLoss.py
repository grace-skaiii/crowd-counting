...
def headCount_inceptionv3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        predict_model = torch.load("./inception_v3_google-1a9a5a14.pth")
        print('start')
        ...
        # model.load_state_dict(torch.load("./inception_v3_google-1a9a5a14.pth"))
...
class Inception3(nn.Module):
    ...
    def __init__(self, num_classes=1000, aux_logits=False, transform_input=False):
        ...
        self.lconv1 = nn.Conv2d(2048, 64, kernel_size = 1, stride=1, padding=0, bias=False)
        self.lconv2 = nn.Conv2d(64, 32, kernel_size = 1, stride=1, padding=0, bias=False)
        self.lconv3 = nn.Conv2d(32, 1, kernel_size = 1, stride=1, padding=0, bias=False)
        # self.lconv4 = nn.Conv2d(64, 64, kernel_size = 3, stride=1, padding=0, dilation=2, bias=False)
        # self.lconv5 = nn.Conv2d(32, 1, kernel_size = 1, stride=1, padding=0, bias=False)
        self.att_conv = nn.Conv2d(2048, 1, kernel_size = 1, stride=1, padding=0, bias=False)
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.conv1_bf_gap = nn.Conv2d(2048, 64, kernel_size = 2, stride=2, padding=0, bias=False)
        self.conv2_bf_gap = nn.Conv2d(64, 5, kernel_size = 1, stride=2, padding=0, bias=False)
        self.GAP = nn.AdaptiveAvgPool2d(1)
        # self.FC1 = nn.Linear(in_features=1024, out_features=64, bias = True)
        # self.FC2 = nn.Linear(in_features=64, out_features=5, bias = True)
        self.PRelu = nn.PReLU()
    ...
    def forward(self, x):
        ...
        # 8 x 8 x 2048
        x = self.upsample(x)
        # density_class = self.conv1_bf_gap(x)
        # density_class = self.conv2_bf_gap(density_class)
        # density_class = self.GAP(density_class)
        ...
        # density_map3 = self.upsample(feature_map3)
        # 32 x 32 x 2048
        density_map3 = self.lconv1(feature_map3)
        # 32x32x64
        density_map3 = self.lconv2(density_map3)
        # 32x32x32
        density_map3 = self.lconv3(density_map3)
        # 32 x 32 x1
        # density_map3 = self.lconv4(density_map3)
        # 24x24x64
        # density_map3 = self.lconv5(density_map3)
        # 32x32x1
        ...
        # print("density_class_predicted:",density_class)
